#!/bin/bash
# Bumps VERSION, builds, and pushes aabutouq/cloudflare-token-sync as vX.X.X.
#
# Usage:
#   ./build-and-push.sh [patch|minor|major]   (default: patch)
#   PUSH=0 ./build-and-push.sh                (build only, skip push)
#
# After a successful run, update the image tag in
# cert-manager/cloudflare-token-sync-cronjob.yaml and commit+push.
set -euo pipefail

IMAGE="aabutouq/cloudflare-token-sync"
...
echo
echo "Done: $tag"
echo "Next: bump the image tag to $new_version in cert-manager/cloudflare-token-sync-cronjob.yaml, then commit+push."
